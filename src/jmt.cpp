...
std::cout << "--- try accel: mod: " << S_mod << " S: " << S << " T: " << T << std::endl;
  } while (S_mod != 0 && S_step > 1e-4);
}
...
//  double a0 = start;
//  double a1 = start_v;
//  double a2 = 0.5 * start_a;
//  double a3 = (12 * a2 + 3 * (end_a - 2 * a2)) * (end_a - 2 * a2) / 36 / (end_v - a1);
//  T = (end_a - 2 * a2) / 6 / a3;
...
//  int steps_count = std::min(int(std::round(T / dt)), max_steps_count);
//  double t = 0;
//  for (int i=0; i<steps_count; i++) {
